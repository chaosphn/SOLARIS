...
@Component({
  selector: 'app-confirmation-customer-dialog',
  standalone: false,
  templateUrl: './confirmation-customer-dialog.html',
  styleUrl: './confirmation-customer-dialog.scss'
})
export class ConfirmationCustomerDialog implements OnInit {
  ...
  private dialogRef  = inject(MatDialogRef<ConfirmationCustomerDialog>);
  ...
  async approve(): Promise<void> {
    try {
      ...
      const result: any = await this.http.updateConfirmationCustomerReview({
        timestamp: this.data.timestamp,
        pointsource: this.data.siteId,
  ...
  async reject(): Promise<void> {
    try {
      this.loading.set(true);
      const result: any = await this.http.rejectConfirmationCustomerReview({
        timestamp: this.data.timestamp,
        pointsource: this.data.siteId,
        reason: this.comment,
        sitename: this.getSiteName(this.data.siteId),
