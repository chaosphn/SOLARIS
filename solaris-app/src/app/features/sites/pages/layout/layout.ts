...
export class Layout implements OnInit, OnDestroy {
  ...
  async getConfig() {
    try {
      const config = await this.http.getConfig2(`assets/site/layout/configurations/layout[${this.siteSelected()}].config.json`);
      if (config) {
  ...
  async getLayoutConfig(){
    const config = await this.http.getConfig2(`assets/site/layout/pv-layout/site[${this.siteSelected()}].config.json`);
    if (config) {
      this.panelList.set(config);
    }
  }

  async getColorConfig(){
    const config = await this.http.getConfig2(`assets/site/layout/pv-layout/color.config.json`);
    if (config) {
      this.colorRange.set(config);
    }
  }
  ...
  getRealtimeRequest(){
    ...
    if(req){
      ...
      this.store.dispatch(LayoutActions.loadLayoutRealtimeData({ requests: sortedReq }));
    }
  }
  ...
  getAttimeRequest(){
    ...
    if(req){
      ...
      this.store.dispatch(LayoutActions.loadLayoutAtTimeData({ requests: sortedReq }));
    }
  }
  ...
  getHistorianRequest(){
    ...
    if(req){
      ...
      this.store.dispatch(LayoutActions.loadLayoutHistorianData({ requests: sortedReq }));
    }
  }
  ...
  async getHistorianData(){
    if (this.requestHistorian() && this.requestHistorian().length > 0) {
      ...
      const res = await Promise.allSettled(result);
      if(res){
        this.store.dispatch(LayoutActions.loadLayoutChartDataSuccess({ data: this.dataChart() }));
        this.store.dispatch(LayoutActions.loadLayoutHistorianDataSuccess({ data: this.dataHistorian() }));
      }
    }
  }
